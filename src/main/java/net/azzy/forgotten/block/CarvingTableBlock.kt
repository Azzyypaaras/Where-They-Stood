...
class CarvingTableBlock(settings: FabricBlockSettings) : BetterFacingBlock(settings, true), BlockEntityProvider {
    ...
    override fun onUse(state: BlockState?, world: World?, pos: BlockPos, player: PlayerEntity?, hand: Hand?, hit: BlockHitResult?): ActionResult {
        ...
        return if (!player.isSneaking && stack.isEmpty) {
            ...
            if (stack.item == ItemRegistry.EMBERS) {
                if (entity.attemptRuneCraft())
                    stack.decrement(1)
                world.playSound(pos.x + 0.5, pos.y + 1.0, pos.z + 0.5, SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.BLOCKS, 1f, 2f, true)
                ActionResult.SUCCESS
            } else if (!player.isSneaking && entity.items[0].isEmpty && !stack.isEmpty) {
