package net.azzy.forgotten.render.util;
...
public abstract class RenderCrimes extends RenderLayer{
    ...
    //            .builder()
    //            .texture(new Texture(texture, true, true))
...
//
    ...
    public static RenderLayer getPortalLayer(Identifier texture, int blendMode) {
        ...
        return of("ff_portal_layer", VertexFormats.POSITION_COLOR, 7, 256, false, true, MultiPhaseParameters.builder().transparency(transparency2).texture(texture2).texturing(new PortalTexturing(blendMode)).fog(BLACK_FOG).build(false));
    }
}
